...
class ImageToPDFConverter:
    ...
    def initialize_ui(self):
        title_label = tk.Label(self.root, text='Image to PDF Converter', font=('Arial', 16, 'bold'), bg='black', fg='#028391')
        title_label.pack(pady=10)

        select_images_button = tk.Button(self.root, text='Select Images', command=self.select_images, bg='black', fg='#028391')
        select_images_button.pack(pady=(0,10))

        self.selected_images_listbox.pack(pady=(0, 10), fill=tk.BOTH, expand=True)

        label = tk.Label(self.root, text='Enter Output PDF Name:', bg='black', fg='#028391')
        label.pack()

        pdf_name_entry = tk.Entry(self.root, textvariable=self.output_pdf_name, width=40, justify='center', bg='black', fg='#028391')
        pdf_name_entry.pack()

        convert_button = tk.Button(self.root, text='Convert to PDF', command=self.convert_images_to_pdf, bg='black', fg='#028391')
...
def main():
    ...
    root.configure(bg='#028391')  # set background color to black
    root.option_add('*Font', ('Arial', 12, 'bold'))  # set font to Arial 12 bold
    root.option_add('*Foreground', 'black')  # set text color to white
